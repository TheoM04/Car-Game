...
func _scroll_to_end() -> void:
	_scroll_to_end_deferred.call_deferred() # better this way, now it scrolls down.
	#call_deferred("_scroll_to_end_deferred")

func _scroll_to_end_deferred() -> void:
	await get_tree().process_frame # will not scroll all the way because its trying to scoll down before it reads the console (for some reasons)
	if output == null:
		return
	output.get_parent().get_v_scroll_bar().value = output.get_parent().get_v_scroll_bar().max_value # this way better, also it's Scroll's fault, not Output.
